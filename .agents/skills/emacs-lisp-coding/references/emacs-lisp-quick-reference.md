...
- Function: `package-function-name`, for example `projectile-find-file`.
- Variable: `package-variable-name`, for example `projectile-project-root`.
- Constant: `package-constant-name`, for example `package-version`.
- Private function: `package--private-function`, for example
  `projectile--get-root`.
- Unused variable: `_variable`, for example `(lambda (x _y) x)`.
- Single-word predicate: `wordp`, for example `integerp` or `stringp`.
- Multi-word predicate: `phrase-p`, for example `buffer-live-p`.
- Face: `package-face-name`, for example `font-lock-keyword-face`.
- Group: `package`, for example `projectile`.
...
- `F`: File name that may not exist.
- `d`: Directory name.
- `b`: Buffer name.
- `B`: Buffer name that may not exist.
- `n`: Number.
- `c`: Character.
- `k`: Key sequence.
- `x`: Lisp expression.
